...
export default function AdminLogin({ data, onBack, onLogin, onRegister }) {
  ...
  const submit = (e) => { e.preventDefault(); if (!data.admin.phone || (phone.replace(/\s/g, '') === data.admin.phone.replace(/\s/g, '') && password === data.admin.password)) onLogin(); else setError(t('invalidAdmin')) }
